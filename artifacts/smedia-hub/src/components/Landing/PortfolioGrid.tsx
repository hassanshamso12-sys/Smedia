...
function PortfolioCardImage({ src, alt, index }: { src: string; alt: string; index: number }) {
  return (
    <>
      {src ? (
        <img
      ...
      <div
        ...
        style={{ display: src ? 'none' : 'flex', background: GRADIENTS[index % GRADIENTS.length] }}
      >
          ...
          {projects.map((project: any, i: number) => (
            <div key={i} className={`glass-hover ${styles.card}`}>
              <div className={styles.imageWrapper}>
                <PortfolioCardImage src={project.image} alt={project.title} index={i} />
