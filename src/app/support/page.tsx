...
export default function SupportDashboard() {
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (loading) {
        console.warn("Firestore fetch timed out, using local defaults.");
        setLoading(false);
      }
    }, 5000); // 5 second timeout

    async function fetchContent() {
      console.log("Attempting to fetch site content from Firestore...");
      try {
        ...
        if (docSnap.exists()) {
          ...
          console.log("No config document found in Firestore, using defaults.");
        }
      } catch (error) {
